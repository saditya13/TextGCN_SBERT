...
# vocab2 = list(vocab2)
# vocab = list(vocab2.union(set(vocab)))
# vocab_size = len(vocab)

#code added end

# word2vec code added starts
...
model_w2v = Word2Vec(size=300, min_count=5, iter = 30)
model_w2v.build_vocab(clean_data)
total_examples = model_w2v.corpus_count
vocab_w2v = list(model_w2v.wv.vocab.keys())
# vocab_w2v = list(model_w2v.wv.index_to_key)
# print("vocab_w2v", vocab_w2v)

model_3 = KeyedVectors.load_word2vec_format('../pretrained word2vec models/GoogleNews-vectors-negative300.bin', binary = True)

model_w2v.build_vocab([list(model_3.vocab.keys())], update=True)
# model_w2v.build_vocab([list(model_3.index_to_key)], update=True)
model_w2v.intersect_word2vec_format("../pretrained word2vec models/GoogleNews-vectors-negative300.bin", binary=True)
model_w2v.train(clean_data, total_examples=total_examples, epochs=model_w2v.epochs)

'''
#''' Word2Vec Without Fine tuning - added by chirag
...
model_w2v = KeyedVectors.load_word2vec_format('../pretrained word2vec models/GoogleNews-vectors-negative300.bin', binary = True)
...
for i, word in enumerate(vocab):
  # vwm = vocab[i], model_w2v.wv[word]


  # Word2vec without Finetuning - added by chirag
  try:
    vec = model_w2v[word]
  except KeyError:
    vec = np.array([0]*word_embeddings_dim)
  # end

  # vec = model_w2v.wv[word]
  word_vector_map[word] = vec
  # vectors.append(vec)
  # vector_word_map.append(vwm)

# word2vec code added  ends
...
for i in range(real_train_size):
    ...
    for word in words:
        if word in word_vector_map:
            ...
            # word_vector = vector_word_map[word]
            # print(doc_vec)
...
# result = list(set(vocab2) - set(vocab))
...
for key in tqdm(word_pair_count):
    ...
    # row1.append(train_size + j)
    # col1.append(train_size + i)
    # weight.append(pmi)
    # pmi_sc.append(pmi)  # added
min_p = min(pmi_sc)
max_p = max(pmi_sc)
...
# pmi_sc = np.array(pmi_sc).reshape(-1,1)
# scaler = preprocessing.MinMaxScaler()
# norm_pmi = scaler.fit_transform(pmi_sc)
# norm_pmi = (norm_pmi.flatten()).tolist()
...
col2= []
cos_sim = []
# word vector cosine similarity as weights (from twignet, added)
from tqdm import tqdm
for i in tqdm(range(int(vocab_size/2))):
# for i in range(int(vocab_size / 2)):
    for j in range(vocab_size):
        if vocab[i] in word_vector_map and vocab[j] in word_vector_map and vocab[i]!=vocab[j]:
            ...
            if similarity > 0.7:
                ...
                # weight.append(similarity)
                cos_sim.append(similarity) # added
                row2.append(train_size + j)
                col2.append(train_size + i)
                # weight.append(similarity)
                cos_sim.append(similarity) # added
'''
...
# for i, n in tqdm(enumerate(len(doc_weight))):
#     doc_weight[i] = (n - min_d)/ (max_d - min_d)
doc_weight = np.array(doc_weight).reshape(-1,1)
scaler = preprocessing.MinMaxScaler()
norm_dw = scaler.fit_transform(doc_weight)
norm_dw = (norm_dw.flatten()).tolist()
doc_mat = sp.csr_matrix(
    (norm_dw, (row3, col3)), shape=(node_size, node_size))
# doc_mat = doc_mat.todense()
# doc_mat = doc_mat + doc_mat.T.multiply(doc_mat.T > doc_mat) - doc_mat.multiply(doc_mat.T > doc_mat)
# row = list(set(row1+row2+row3))
# col = list(set(col1+col2+col3))
adj = sp.csr_matrix(weight_dense + doc_mat.todense())
