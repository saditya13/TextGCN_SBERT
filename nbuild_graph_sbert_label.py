...
if((similarity!='cos') and (similarity!='euc')):
...
# tweet_embeddings = model.encode(shuffle_doc_words_list) #random comparison
tweet_embeddings = model.encode(doc_train_data) #serial comparison
...
if(similarity=='cos'):
    ...
    sim_docs = []
    var = np.arange(0,1000) # for storing random 1000 pairs in file
    k=0
    num =0
    ...
    for i in tqdm(range(train_size)):
        for j in range(i+1,train_size):
            #if value is above threshold, add the edge. It is stored as a sparse matrix, first x component, then y and then the weight is added.
            if (cos_sim[i][j].item() > threshold):
            # if(cos_sim[i][j] > threshold) and \
            #         shuffle_doc_name_list[i].split('\t')[2] == shuffle_doc_name_list[j].split('\t')[2]:
                if i < train_size:
                    row.append(i)
                else:
                    row.append(i + vocab_size)
                ...
                # # if k < 1000 :
                #     # s_l= doc_labels[i],doc_labels[j]            #for serial comparison
                #     sim_docs.append([k,i,doc_train_data[i],j,doc_train_data[j], s_l, cos_sim[i][j].item()])
                #     # sim_docs.append([k,shuffle_doc_words_list[i],shuffle_doc_words_list[j], s_l, cos_sim[i][j]])
                #     k = k+1
    ...
    for i in tqdm(range(len(shuffle_doc_words_list))):
        for j in range(i+1,len(shuffle_doc_words_list)):
...
# import csv
# with open(dataset + '.sim.csv', 'w') as f_out:
...
print("Number of new edges created: ", num)
